<template>
    <div class="app-container">
        ...
        <div class="filter-container">
            <el-input v-model="filterForm.name" clearable :placeholder="$t('filterForm.permissionName')" style="width: 150px;" class="filter-item" />
            <el-button v-waves class="filter-item" type="success" icon="el-icon-search" @click="search">{{ $t('filterForm.search') }}</el-button>
            <el-button v-if="hasAuthority('PERMISSION_ADD')" class="filter-item" type="primary" icon="el-icon-plus" @click="add" style="margin-left: 0px;">{{ $t('filterForm.add') }}</el-button>
            <el-button v-if="hasAuthority('USER_DELETE')" class="filter-item" type="danger" icon="el-icon-delete" @click="_delete" style="margin-left: 0px;">{{ $t('filterForm.delete') }}</el-button>
            <el-button v-if="hasAuthority('PERMISSION_GET')" v-waves :loading="filterExporting" class="filter-item" type="warning" icon="el-icon-download" @click="handleExport" style="margin-left: 0px;">{{ $t('filterForm.export') }}</el-button>
        </div>
        ...
        <el-table
            ...
            <el-table-column label="权限名" align="left" prop="name" show-overflow-tooltip min-width="120">
            </el-table-column>
            ...
            <el-table-column label="操作" align="center" width="140" class-name="small-padding fixed-width">
                <template slot-scope="{row}">
                    ...
                    <el-button v-if="hasAuthority('PERMISSION_UPDATE')" type="primary" circle icon="el-icon-edit" @click="update(row)" style="margin-left: 5px;"/>
                    <el-popover
                            v-if="hasAuthority('PERMISSION_DELETE')"
                            :ref="row.id"
                            placement="top"
                            width="180">
        ...
        <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" append-to-body width="880px" >
            <el-form ref="dialogFormRef" :model="dialogTemp" :rules="dialogRules" :inline="true"  label-position="left" label-width="140px">
                <el-form-item label="权限名" prop="name">
                    <el-input v-model="dialogTemp.name" placeholder="请输入权限名" style="width: 675px;"/>
                </el-form-item>
                <el-form-item label="备注" prop="remark">
                    <el-input v-model="dialogTemp.remark" placeholder="请输入备注" type="textarea" max="255" :autosize="{ minRows: 2, maxRows: 4}" style="width: 675px;"/>
                </el-form-item>
            </el-form>
    ...
    export default {
        ...
        data(){
            return{
                ...
                dialogRules: { // 对话框验证规则
                    name:[{ required: true, message: '不能为空!', trigger: 'blur' }, {max: 255, message: '权限名最长255位', trigger: 'change' }],
        ...
        methods:{
            ...
            handleAdd(){
                this.$refs['dialogFormRef'].validate((valid) => {
                    if (valid) {
                        ...
                        SystemPermissionApi.add(this.dialogTemp).then(res => {
                            this.$message({
                                message: '增加成功!',
                                type: 'success'
                            });
            ...
            handleDelete(ids){
                this.$confirm('此操作将永久删除数据, 是否继续?', '提示', {
                    ...
                    SystemPermissionApi._delete(ids).then(res => {
                        this.$message({
                            message: '删除成功!',
                            type: 'success'
                        });
            ...
            handleExport(){
                ...
                import('@/utils/excelUtil').then(excel => {
                    ...
                    excel.export_json_to_excel({
                        header: tHeader,
                        data,
                        filename: 'InvestmentFiling Permission List'
                    })
